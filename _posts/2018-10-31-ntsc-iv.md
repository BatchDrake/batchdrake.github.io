...
The coefficient \\(\beta=1-\alpha\\) of aforementioned low pass filter is also known as the _decay factor_ and, since it is a recursive filter, its impulse response must be infinite. In fact, the analytic expression of its impulse response looks like this:
