...
Since these bursts are again too short, their time inspection make sense. We can do it either by pressing the "Hold" button until they show up or by holding "Autosequelch" for a brief period of time (enough to measure the noise floor). Posterior analysis revealed that all bursts were basically the same burst repeated again and again, so it's enough to isolate just one.

By analyzing the signal in the spectrogram, we can tell that although most of the energy (and probably the information) is concentrated in a main lobe roughly 24 kHz wide, it is not perfectly band-limited: there's a bit of extra energy expanding the whole spectrum around the burst, decreasing with the distance to the central frequency. This out-of-band energy, which is actually redundant with respect to the true information contained in the burst, is called the **skirt** because of its shape and it is due to the particular pulse shaping function used in the signal, which is not band-limited either.

![]({{ site.baseurl }}/images/cts-signal4-narrow.png) 
<center><small><em>Beginning of the burst, 24 kHz</em></small></center>


By clicking on "Envelope", a shape fitting the immediate amplitude of the signal is drawn behind its waveform. Its rectangular shape let us conclude that it is a constant-envelope signal and therefore cannot be amplitude modulated (like the signal in the previous challenge).

![]({{ site.baseurl }}/images/cts-signal4-envelope.png) 
<center><small><em>Envelope of the burst</em></small></center>


Since it is a constant envelope signal, it must be either frequency or phase modulated. Since we are still in the early stages of the contest and frequency-modulated signals are somewhat easier to demodulate than phase-modulated signal, we can go ahead and study the instantaneous frequency of the samples in the burst. To achieve this, we should go to "Sampling and decision", choose "Frequency" in the Decision space and, after making sure that the burst limits are selected, click in "Histogram". A window like this should show up:

![]({{ site.baseurl }}/images/cts-signal4-hist.png) 
<center><small><em>Envelope of the burst</em></small></center>


The histogram reveals that two instantaneous frequencies, around 4.75 Hz apart, are particularly frequent. This is a strong evidence in favour of a frequency modulated signals. The irregular and weaker in-between peaks could be due to the pulse-shaping function and may not carry information. 


In order to test our hypothesis, we can use the Time Window to draw the instantaneous frequency of the signal directly in the envelope box and see whether we see some kind of regularity.

> **Protip:** We are about to perform precise time measurements on the signal. Although it should be sufficient to fit the filter box to the main lobe, in this particular case where other out-of-bands signals are absent, I recommend extending the filterbox to the whole spectrum (effectively increasing the sample rate of the selected channel and therefore the time resolution of the captured burst, which ultimately leads to more precise time measurements). The rationale is that although the skirt is redundant, it may contain pulse edge information what makes the visual inspection of the waveform even clearer.
...
This suggest frequency-modulated pulses varying between two frequencies. The narrowest pulses are roughly the same in width, suggesting a fixed symbol clock (as with most digital signals). This hypothesis can be furtherly validated with the periodic selection feature as we did with out previous challenge, indeed:
...
> **Note:** The fact that both frequencies are 4.75 Hz apart (which is roughly half the baudrate) suggests we are dealing with [MSK](https://en.wikipedia.org/wiki/Minimum-shift_keying). Also, the skirt suggest the presence of [ISI](https://en.wikipedia.org/wiki/Intersymbol_interference) which is a common trait of a very popular modulation named [GMSK](https://en.wikipedia.org/wiki/Minimum-shift_keying#Gaussian_minimum-shift_keying) used, for instance, in GSM networks (although at a way higher rate). The gaussian filter would be the one to blame for the long skirt of the bursts.

Since this is a burst, we can simply select the burst with the mouse, and perform a Gardner clock recovery in the frequency space. This time, since one frequency is negative while the other is positive, the decision threshold is exactly 0 and no further adjustments are necessary. We did it for illustration purposes nonetheless:

![]({{ site.baseurl }}/images/cts-signal4-sampling.png) 
<center><small><em>Instantaneous frequency of the signal.</em></small></center>


Repeating the same strategy again, setting the row size to 8, shows again a row full of zeroes. Strong evidence that data is encoded in a similar fashion. By exporting the symbols to a C file and adding a function to repack the bits according to different offsets, we came up with the following code:
...
Being the 3 ```aa``` the set of 24 pulses of the preamble / training sequence in the beginning of the burst and `0x850d32f1` the syncword referred in the flag of the previous challenge. These signs were taken, in a first step, as strong evidence that we were in the right track. However, the payload did not make any sense and struggled for hours until reaching a dead end. We had to retrace our steps to the demodulation phase and give a good look to the frequency histogram which this time looked like this:
...
This has strong implications in the demodulation process. To begin with, we have to select **2** bits per symbol in the Sampling Window. In this case, the correct adjustment of the decision thresholds is mandatory as every component must fall exactly in the middle of each decision interval:

![]({{ site.baseurl }}/images/cts-signal5-sampling.png) 
<center><small><em>Signal 5's histogram</em></small></center>


If we export the data to a C file, what we get in the array is not bits but symbol identifiers (actually we always export symbols, but in the previous cases symbol identifiers mapped to the bits they represented). Having 2 bits per symbol implies having an alphabet of 2²=4 symbols and therefore 4!=24 possible symbol-2bit translations.

In real world communications, when a symbol represents more than 1 bit, Gray coding is preferred, so the number of possible translations is actually way lesser than the factorial of a power of two.  In our case and due to our frustration, we wrote a code that tested all 24 possible symbol translations, packing bits with MSB first. The resulting code looked as follows:
...
The signal in 1.45 GHz as exactly like the one in the third challenge, OOK at 10 bits per second, with a 2-bit length field after the syncword. However, it was Friday night already, GnuRadio (both 3.7 and 3.8) refused to work in my computer and had to use GR 3.7 that was installed by chance on other computer. In any case, after a lot of trial-and-error, we arrived to a flowgraph that aligned the bit stream to the syncword and dumped everything to the standard output that ended up not being accepted because it didn't take the length into account.
