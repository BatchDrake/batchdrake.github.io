...
From which we can deduce it is basically 2-FSK signal at 9600 bits per second. We can go ahead and sample it as we did with our previous signal. Since this is a burst, we can simply select the burst with the mouse, and perform a Gardner clock recovery in the frequency space. This time, since one frequency is negative while the other is positive, the decision threshold is exactly 0 and no further adjustments are necessary. We did it for illustration purposes nonetheless:
